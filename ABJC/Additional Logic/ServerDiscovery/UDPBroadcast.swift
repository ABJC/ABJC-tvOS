...
open class UDPBroadcastConnection {
    ...
    /// - Note: This doesn't open a socket!
    /// The socket is opened transparently as needed when sending broadcast messages.
    /// If you want to open a socket immediately, use the `bindIt` parameter.
    /// This will also try to reopen the socket if it gets closed.
    ///
    /// - Parameters:
