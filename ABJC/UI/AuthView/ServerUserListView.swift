//
//  ServerUserList.swift
//  ABJC
//
//  Created by Stephen Byatt on 3/5/21.
//
...
extension AuthView.ServerSelectionView {
    ...
    struct UserImageBoxView: View {
        ...
        private var url: URL? {
            return API.profileImageURL(for: user, jellyfin)
        ...
        var body: some View {
            VStack {
                image
                    .overlay(Image(systemName: user.hasPassword ? "lock" : "lock.open" ), alignment: .bottomLeading)
                Text(user.name)
                    .padding(.bottom)
            }
        }
